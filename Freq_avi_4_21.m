...
vidWidth = obj.Width;
   ...
   numFrame=obj.Duration*obj.FrameRate;
   ss=zeros(vidHeight,vidWidth,3,numFrame);
...
ssgray=reshape(ssgray,[vidHeight vidWidth numFrame]);
ssdvalue=ssgray;
for i=1:numFrame
    ssdvalue(:,:,i)=filter2(fspecial('average',7),ssgray(:,:,i))/255;
...
OFF_period(i,:) = OFF_period(i,:)+Offset;
end
...
REC_period(i,2) = find((t<=REC_pairs(i,2)),1,'last'); % find the end poingt for REC period i
 REC_period(i,:) = REC_period(i,:)+Offset;
end
for i=1:size(REC_pairs,1)
